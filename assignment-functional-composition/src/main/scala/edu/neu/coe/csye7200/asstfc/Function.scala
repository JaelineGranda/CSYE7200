...
object Function {
  ...
  def lift2[T1, T2, R](f: (T1, T2) => R): (Try[T1], Try[T2]) => Try[R] =
    for (t1: T1 <- _; t2: T2 <- _) yield f(t1,t2)
  ...
  def lift3[T1, T2, T3, R](f: (T1, T2, T3) => R): (Try[T1], Try[T2], Try[T3]) => Try[R] =
    for (t1: T1 <- _; t2: T2 <- _; t3: T3 <- _) yield f(t1, t2, t3)
  ...
  (Try[T1], Try[T2], Try[T3], Try[T4], Try[T5], Try[T6], Try[T7]) => Try[R] =
    for (t1: T1 <- _; t2: T2 <- _; t3: T3 <- _; t4: T4 <- _; t5: T5 <- _; t6: T6 <- _; t7: T7 <- _) yield f(t1, t2, t3, t4, t5, t6, t7)
